...
我们把`lambda`演算中的`f`和`x`分别取为`countTime`和`x`，就得到了我们的自然数。这也说明了不管你使用`符号系统`还是`JavaScript`语言，你想要表达的`自然数`，是等价的。这是一种形式上的抽象！
...
定义`函数的元`的意义在于，我们可以把函数进行归类了，并且可以明确一个函数需要的确切参数个数。如果我给你一个`二元函数`，你就知道需要传递两个参数。比如`+`就可以看成是一个`二元函数`，它的左边接受一个参数，右边接受一个参数，返回它们的和（或`连接`）。

但我们上面看到的`λ演算`，每个函数都只有一个`元`。为什么呢？

只有一个`元`的函数方便我们进行代数运算。`λ演算`的参数列表使用`λx.λy.λz`的格式进行分割，返回值一般都是函数，如果一个`二元函数`，调用时只使用了一个参数，则返回一个`不完全调用函数`。我有三个例子解释`不完全调用`
...
在`js`中，也成立！`不完全调用`实际上就是`JavaScript`中闭包产生的原因，一个函数还没有被销毁（调用没有完全结束），你可以在子环境内使用父环境的变量！
...
柯里化（`curry`）函数是一种把函数的元降维到一元的技术，这个名词是为了纪念我们上文提到的数学家阿隆佐·邱奇。
...
上面的方法假设我们已经有了一些基础函数`toBinary`和`toStringOx`，并且有一种方式把taken组合起来`pipe`，这样我们的数据类型转换用`Typescript`表示就是
...
综上，`Point-Free`风格是粘合一些基础函数，最终让我们的数据操作不再关心中间态的方式。如果非要翻译的话，应该是`无值`或`无参`风格？

## 函数式特性

说了这么久，都是在讲函数，那么究竟什么是函数式编程呢？在网上你可以看到很多定义，但大都离不开这些特性

- 函数为第一等公民：函数可以被应用，也可以被当作数据
- 纯函数，无副作用：任意时刻以相同参数调用函数任意次数得到的结果都一样
- 引用透明：可以被表达式替代
- 基于表达式：表达式可以被计算，促进数据流动，状态声明就像是一个暂停，好像数据到这里就会停滞了一下（声明影响了我拔刀的速度。）
- 不可变性：参数不可被修改、变量不可被修改---宁可牺牲性能，也要产生新的数据（`rust`内存模型例外）
- 大量使用高阶函数：变量存储、闭包应用、函数高度可组合
- 柯里化：

另外还有一些特性，有的会提到，有的一笔带过，但实际也是一个特性（以`haskell`为例）

- 类型推导：如果无法确定数据的类型，那函数怎么去组合？
- 惰性求值：函数天然就是一个执行环境，惰性求值是很自然的选择。
- IO：一种类型，用于处理副作用。一个不能执行打印文字、修改文件等操作的程序，是没有意义的，总要有位置处理副作用。（边缘）
...
这个`add`函数是不纯的，但我们把副作用都放到它里面了。任意使用这个`add`函数的位置，都将变成不纯的。
...
没有类型推导的函数式编程是瘸腿的，在`js`很多库中都对类型坐了限制，对象参数传递了数组，则可能拿不到值。
...
我们要在一个更大的空间上讨论这个对象。就像`Number`封装了数字类型，`Monad`也封装了一类型。

```javascript
// Just是一个单子，可以在单子范畴上进行映射，映射结果自然也是单子。
function Just(__value) {
    return {
        fmap(fn) {
            return Just(fn(__value))
        },
        __inspect() {
            return __value
        }
...
有了`Just`的概念，我们再来学习一些新的`monad`。比如`Nothing`
...
出了这种值存在与否的判断，我们的程序还有一些分支结构的方式，因此我们来看一下在`monad`空间中，分支情况怎么去模拟？
...
终于到`IO`了，如果不能处理好`IO`，我们的程序是无法健全的。到目前为止，我们的monad都是针对数据的。这句话对也不对，因为函数也是一种数据！（函数是第一公民）。我们先让`Just Monad`能存储函数。
